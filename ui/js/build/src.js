...
App.managers.StoreManager = {
    load: function load(type, lat, lon) {
        $.ajax({
    ...
    update: function update() {
        ...
        App.data.stores.filter(function (store) {
            return bounds.contains(L.latLng(store.lat, store.lon)) && !store.rendered;
        }).forEach(function (store) {
            L.marker(store, { title: store.name, alt: "Store marker: ${store.name}" }).addTo(App.data.map).bindPopup(App.managers.StoreManager.popupContent(store));
            store.rendered = true;
        });
    },

    loadFromCenter: function loadFromCenter(type) {
        var center = App.data.map.getCenter();
        this.load(type, center.lat, center.lng);
    },

    popupContent: function popupContent(store) {
    ...
    onAdd: function onAdd(map) {
        ...
        function build_dropdown(points) {
            ...
            points.forEach(function (point) {
                btn_list.append($("<li></li>").append($("<a></a>").attr("href", "#").text(point.name)).click(function () {
                    map.panTo(point);
        ...
        var container = $("<div></div>").addClass("btn-group").append($("<button></button>").addClass("btn btn-default dropdown-toggle").attr("type", "button").attr("data-toggle", "dropdown").attr("aria-haspopup", "true").attr("aria-expanded", "false").html(points[0].name + " <span class=\"caret\"></span>")).append(build_dropdown(points));
        return container[0];
    }
});
